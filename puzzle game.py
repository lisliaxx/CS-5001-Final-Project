...
for i in range(row):  # Put all the images into correct order
    for j in range(column):
        tile = images[i * row + j]
        tile.left = j * tile_size  # tile.left shows the location number of the image left end on x-axis
        tile.top = i * tile_size  # tile.left shows the location number of the image left end on y-axis
...
def draw():  # Images, Text, Shapes that needed to be placed on the screen
    ...
    if win:
        ...
        if image_first == -1:
            ...
            screen.draw.rect(
                Rect((images_list[image_first].left, images_list[image_first].top), (tile_size, tile_size)),
...
def on_mouse_down(pos):  # When clicking the screen
    ...
    for i in range(row):
        for j in range(column):
            ...
            if selected_image.left != j * tile_size or selected_image.top != i * tile_size:  # Compare positioning
                win = False
                break
